            ...
            : this(new TSettings(), new TSerializer(), new TTransformer(), new TGenerator(), new TNamer())
        {
            Context = new Context
            {
                () =>
                {
					...
					Singleton.SetInstance(CodeNamer,typeof(CodeNamer));
					Singleton.SetInstance(CodeNamer,typeof(TNamer));

                    // and the language specific settings
					Singleton.SetInstance(Settings,typeof(TSettings));
					Singleton.SetInstance(Settings,typeof(IGeneratorSettings));
                }
            };
        }
